...
newManufacturers stock_ ( _, sect ) =
  List.foldl
    (\listing ( stock, names ) ->
      let
        manufacturer =
          Listing.description listing |> .manufacturer
      in
...
init encodedFlags url key =
  let
    flags =
      Decode.decodeValue flagsDecoder encodedFlags
        |> Result.toMaybe
        |> Maybe.withDefault
            { protocol = "https://"
  ...
  case msg of
    ...
    UrlChanged url ->
      let
        updatedModel =
          { model
            | navi = NavState model.navi.key url (Route.fromUrl url)
          }
      in
      case updatedModel.navi.route of
        Route.Products category ->
          let
            loadState =
              updatedModel.store
                |> getStoreSectionLoadState category
          in
          case loadState of
            Initializing ->
            ...
            FailedInitializing _ ->
              ( updatedModel
              , getProductsJson model.flags Nothing category
              )

            FailedUpdating etag _ ->
              ( updatedModel
              , getProductsJson model.flags (Just etag) category
              )
    ...
    GotProducts res ->
      case res of
        Ok ( category, change, newProducts ) ->
          let
            sect =
              getStoreSection category model.store
          in
          case change of
            NoChange ->
              ( { model
    ...
    GotStock res ->
      case res of
        Ok ( manufacturer, change, newStock ) ->
          let
            sect =
              getStockSection manufacturer model.stock
        ...
        Err ( manufacturer, err ) ->
          case err of
            Http.BadBody "availability-empty" ->
              let
                etag =
                  getStockSectionLoadState manufacturer model.stock
...
viewProducts stock products =
  List.map
    (\listing ->
      ...
      in
      Listing.viewKeyed ( listing, viewData )
    )
    products
    |> Keyed.ul []
    ...
    \res ->
      case res of
        ...
        Http.GoodStatus_ metadata body ->
          ...
          let
            errorModesActive =
              Dict.get "x-error-modes-active" metadata.headers
                |> Maybe.withDefault ""
          in
...
ifNoneMatchReq { protocol, rootUrl, corsProxy } etag urlString expectFunction =
  let
    ifNoneMatch =
      etagIfNoneMatch (Maybe.withDefault etagEmpty etag)
  in
  Http.request
    { method = "GET"
    , headers = [ ifNoneMatch ]
